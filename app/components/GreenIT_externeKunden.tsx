...
export default function AssetpreisKalkulator({ config, kundennummer }: Props) {
 ...
 const safeNum = (v: number | "" | undefined | null) =>
  ...
  const totalPallets = quantities.reduce((acc: number, val, idx) => {
  ...
  // ======= ✅ EIN finaler Preis nach außen (intern + frei eingabbar), ohne Ausweisung =======
 const total = transportCost + totalMaterial + totalPackaging;

// interner Zuschlag (nicht sichtbar)
const INTERNAL_SURCHARGE_RATE = 0.04;
const ADDITIONAL_SURCHARGE_RATE = 0.05;

// erst +4 %, dann +5 %
const totalWithSurcharge =
  total * (1 + INTERNAL_SURCHARGE_RATE) * (1 + ADDITIONAL_SURCHARGE_RATE);

const avgUnitPrice =
  totalQuantity > 0
    ? (totalWithSurcharge / totalQuantity).toFixed(2)
    : "0.00";

const totalCost = totalWithSurcharge.toFixed(2);
  ...
  const mailTo = (() => {
    ...
    const subject = `${rechnungsadresse.firmenname}${
      reference ? " – " + reference : ""
    }`;
    ...
    const bodyLines = [
      ...
      // ✅ nur ein Gesamtpreis – keine Aufschlagsausweisung
  ...
  const getPdfHtml = () => {
    ...
    return `
      ...
      <body>
        <h2>Kalkulation – ${rechnungsadresse.firmenname}${
          reference ? " – " + reference : ""
        }</h2>
        <table>
          <thead>
        ...
        <hr />
        <p>Rechnungsadresse: ${rechnungsadresse.firmenname}, ${
        ...
        <p>Abholadresse: ${abholadresse.firmenname}, ${abholadresse.strasse}, ${
          abholadresse.plz
        } ${abholadresse.ort}, ${abholadresse.land}</p>
        <div class="remarks-box">
          <b>Bemerkungen:</b><br/>
          ${
            bemerkungen.trim()
              ? bemerkungen.trim().replace(/\n/g, "<br/>")
              : "Keine"
          }
        </div>
      </body>
      ...
      <form>
        ...
        {showBanner && (
          <div className="banner-warning">
            Bei Palettenanzahl ab 26 erhalten Sie ein individuelles Angebot.
            Bitte senden Sie Ihre Anfrage per E-Mail an{" "}
            <a href="mailto:dispo@logist.de">dispo@logist.de</a>.
          </div>
        )}
        ...
        <fieldset>
          ...
          <input
            ...
            value={rechnungsadresse.firmenname}
            disabled
          />
          <input
            name="strasse"
            placeholder="Straße"
            value={rechnungsadresse.strasse}
            disabled
          />
        ...
        <fieldset>
          ...
          <input
            name="firmenname"
            placeholder="Firmenname"
            value={abholadresse.firmenname}
            onChange={(e) => handleAdresseChange(setAbholadresse, e)}
            required
          />
          <input
            name="strasse"
            placeholder="Straße"
            value={abholadresse.strasse}
            onChange={(e) => handleAdresseChange(setAbholadresse, e)}
            required
          />
          <input
        ...
        {abweichendeLieferadresse && (
          <fieldset className="zieladresse-feldset">
            ...
            <input
              name="firmenname"
              placeholder="Firmenname"
              value={zieladresse.firmenname}
              onChange={(e) => handleAdresseChange(setZieladresse, e)}
              required
            />
            <input
              name="strasse"
              placeholder="Straße"
              value={zieladresse.strasse}
              onChange={(e) => handleAdresseChange(setZieladresse, e)}
              required
            />
            <input
